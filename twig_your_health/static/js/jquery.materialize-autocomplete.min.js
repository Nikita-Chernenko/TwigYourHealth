(function (root, factory) {
    ...
    Autocomplete.defaults = {
        ...
        limit: 10,
        multiple: {
            enable: false, maxSize: 4, onExist: function (item) {
                Materialize.toast("Tag: " + item.text + "(" + item.id + ") is already added!", 2e3)
            }, onExceed: function (maxSize, item) {
                Materialize.toast("Can't add over " + maxSize + " tags!", 2e3)
        ...
        hidden: {enable: true, el: "", inputName: "", required: false},
        appender: {
            el: "",
            tagName: "ul",
    ...
    Autocomplete.prototype = {
        constructor: Autocomplete, initialize: function () {
            var self = this;
            var timer;
            var fetching = false;
            ...
            self.$el.on("keydown", function (e) {
                ...
                if (keyCode == "13") {
                    ...
                    self.setValue({id: $hover.data("id"), text: $hover.data("text")});
                    return false
                }
            });
            self.$dropdown.on("click", "[data-id]", function (e) {
                var $t = $(this);
                var item = {id: $t.data("id"), text: $t.data("text")};
                self.setValue(item)
            });
            self.$appender.on("click", "[data-id] .close", function (e) {
                var $t = $(this);
                var $li = $t.closest("[data-id]");
                var item = {id: $li.data("id"), text: $li.data("text")};
                self.remove(item)
            })
        }, render: function () {
            var self = this;
